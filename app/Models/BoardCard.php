...
class BoardCard extends Model
{
    ...
    protected $fillable = [
        'board_id', 'user_id', 'list_id', 'card_title', 'card_description',
    ];

    protected $attributes = [
        'card_description' => '' // Valor padrão vazio
    ];
    ...
    public function createCard($input, $user_id)
    {
        return $this->create([
            ...
            'card_description' => $input->get('card-title', '')
        ]);
    }
    ...
    public function updateCard($input)
    {
        $this->where('id', $input->get("cardId"))->update([
            ...
            "card_description" => ($input->get("cardDescription") != "Nada foi registrado. Clique e adicione uma descrição.") ? $input->get("cardDescription") : '',
    ...
    public function getBoardCards()
    {
        return $this->select([
            ...
            ->groupBy(['board_card.id', 'users.name']) // Adicione users.name aqui
            ->get();
    }
    ...
    public function cardTotalTask()
    {
        return $this->select([
            ...
            ->get()
            ->map(function ($item) {
                // Garante que totalTasks exista como inteiro
                $item->totalTasks = (int) ($item->totalTasks ?? 0);
                return $item;
            });
    }
